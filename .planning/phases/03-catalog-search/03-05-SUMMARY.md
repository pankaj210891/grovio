...
decisions:
  - "jose added to @grovio/api package.json — was in packages/contracts but not in the api app's direct deps; TS typecheck failed without it"
  - "TDD mock pattern: SignJWT and S3Client/PutObjectCommand mocked as ES classes (not vi.fn() factories) because they are used with `new` — factory mocks cause 'is not a constructor' TypeError"
  - "ProductService accepts featureFlagService and productIndexQueue as injected deps typed as Pick<> — prevents import cycles with feature-flags and jobs modules"
  - "ImageService: S3ClientConfig constructed with spread to omit undefined optional properties — required for exactOptionalPropertyTypes: true tsconfig"
...
Three tasks executed in TDD (RED → GREEN) order:
...
- `VendorAuthService.test.ts`: 11 unit tests covering all behaviors (SignJWT mocked as ES class; jwtVerify mocked as vi.fn()).
- Added `jose` to `apps/api/package.json` — was missing from API's direct dependencies despite being used.

**Task 2 — ProductService + catalog module barrel** (`apps/api/src/modules/catalog/`)

- `ProductService.ts`:
  - `createProduct(vendorId, input)`: loads category + restriction check (D-18/CAT-06) → validates attributes against `attribute_definitions` schema registry (T-03-P4) → generates unique slug (collision loop) → inserts with status='draft'.
  - `updateProduct(id, vendorId, input)`: ownership guard (`eq(products.vendorId, vendorId)` — V4) → if current status is 'pending_review', resets to 'draft' before applying updates (D-06, Pitfall 3) → NEVER writes status directly.
  - `submitProduct(id, vendorId)`: from 'draft' only → reads `CATALOG_AUTO_APPROVE` via `featureFlagService.getFlag()` → 'true' → status='approved' + index job; 'false' → status='pending_review' (D-07).
  - `approveProduct(id)`: from 'pending_review' → status='approved' + `productIndexQueue.add('index', { productId, action:'index' })` (D-14).
  - `rejectProduct(id, rejectionReason)`: requires non-empty reason (D-08) → status='rejected' + stores reason → if was 'approved', enqueues delete job (D-13, Pitfall 7).
  - `archiveProduct(id, vendorId)`: ownership guard → archivedAt=now + delete job (Pitfall 7).
  - `listVendorProducts(vendorId, cursor?, limit?)`: cursor pagination (createdAt DESC, id DESC), vendor-scoped, non-archived.
  - Domain errors: `ProductStateError`, `ProductOwnershipError`, `ProductNotFoundError`, `ProductRestrictionError`, `ProductValidationError`.
- `index.ts`: barrel re-exporting ProductService, ImageService, and all error classes.
- `ProductService.test.ts`: 17 unit tests covering all behaviors with featureFlagMock and queueMock.

**Task 3 — ImageService** (`apps/api/src/modules/catalog/`)

- `ImageService.ts`:
  - `generatePresignedUpload(productId, vendorId, { contentType, fileSizeBytes })`: ownership check → `ImageSizeError` if > MAX_IMAGE_SIZE_BYTES → `ImageLimitError` if count >= MAX_IMAGES_PER_PRODUCT → generates key as `products/${productId}/${uuid}` → `getSignedUrl(s3, PutObjectCommand, { expiresIn: 300 })` → cdnUrl = `${S3_PUBLIC_URL}/${key}`. Returns `{ uploadUrl, cdnUrl, key }`. WRITES NOTHING to DB (Pitfall 4, T-03-P6).
  - `confirmUpload(productId, vendorId, { key, altText })`: ownership check → inserts product_images row. Only DB-write method (Pitfall 4).
  - `reorderImages(productId, vendorId, orderedImageIds)`: ownership check → batch-update sortOrder by index.
  - `deleteImage(imageId, productId, vendorId)`: ownership check → hard-delete product_images row.
  - S3Client initialized lazily from env; config built with spread to skip undefined optional props (exactOptionalPropertyTypes).
  - Domain errors: `ImageLimitError`, `ImageOwnershipError`, `ImageSizeError`.
...
- **Issue:** `jose` was in `packages/contracts/package.json` (or accessible via hoisting) during Phase 1/2 but was not in `apps/api/package.json`. TypeScript threw TS2307 "Cannot find module 'jose'" during typecheck.
- **Fix:** Ran `pnpm --filter @grovio/api add jose` to add it as a direct dependency. This is a legitimate package (npmjs.com/package/jose, 6M+ weekly downloads, Anthropic/auth0 maintained).
...
- **Found during:** Task 1 GREEN + Task 3 GREEN — `vi.fn().mockImplementation(() => ({...}))` caused "TypeError: is not a constructor" for `new SignJWT()`, `new S3Client()`, `new PutObjectCommand()`.
...
- **Found during:** Task 3 typecheck — `endpoint: string | undefined` not assignable to `S3ClientConfig.endpoint` under `exactOptionalPropertyTypes: true`.
- **Fix:** Changed to spread-based config construction: `...(env.S3_BUCKET_URL ? { endpoint: env.S3_BUCKET_URL } : {})` — omits the property entirely when undefined rather than setting it to undefined.
- **Files modified:** `ImageService.ts`
- **Commit:** 2129917

**4. [Rule 1 - Bug] jwtVerify mock needed `as never` cast for TypeScript TS2345**
- **Found during:** Task 1 typecheck — `vi.mocked(jose.jwtVerify).mockResolvedValueOnce({ ... })` object was missing the `key` field required by `ResolvedKey` type in jose 6.x.
- **Fix:** Added `as never` cast to the mock return value — correct pattern for mocking overloaded library functions in TypeScript.
- **Files modified:** `VendorAuthService.test.ts`
- **Commit:** 222a8b3

## Threat Surface Scan

New catalog API service layer introduced. Routes not yet created (Plan 03-07). No new HTTP endpoints exposed in this plan.

| Flag | File | Description |
|------|------|-------------|
| threat_flag: vendor-jwt-issuance | VendorAuthService.ts | Vendor JWT issuance path — mitigated: argon2id hash verify, same error message for unknown-email/wrong-password (T-03-P1) |
| threat_flag: product-state-bypass | ProductService.ts | Product status mutation path — mitigated: only named methods change status; updateProduct cannot set status except pending_review→draft (T-03-P2) |
| threat_flag: product-idor | ProductService.ts | Product ownership path — mitigated: all vendor mutations filter by eq(products.vendorId, vendorId) (T-03-P3/V4) |
| threat_flag: image-size-dos | ImageService.ts | Image upload size path — mitigated: MAX_IMAGE_SIZE_BYTES at presign time + ContentLength on PutObjectCommand (T-03-P5) |
| threat_flag: image-orphan | ImageService.ts | Image presign-but-no-confirm path — mitigated: presign writes nothing; only confirmUpload inserts; 5min presign TTL (T-03-P6) |

## Known Stubs

None. These are pure service implementations with no placeholder data, hardcoded values, or UI components.
...
- VendorAuthService.ts contains argon2 import — FOUND
- VendorAuthService.ts contains SignJWT / jwtVerify — FOUND
- VendorAuthService.ts contains role:'vendor' — FOUND
- ProductService.ts contains pending_review reset to draft — FOUND
- ProductService.ts contains CATALOG_AUTO_APPROVE — FOUND
- ProductService.ts contains productIndexQueue — FOUND
- ImageService.ts contains getSignedUrl — FOUND
- ImageService.ts contains PutObjectCommand — FOUND
- ImageService.ts contains MAX_IMAGE_SIZE_BYTES check — FOUND
