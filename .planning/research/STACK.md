...
**Overall confidence:** HIGH — all versions verified against npm registry or official release pages; rationale cross-checked against ARCHITECTURE.md and PITFALLS.md
...
**Monorepo layout** (from ARCHITECTURE.md — do not change this):

```
grovio/
├── apps/
│   ├── api/               # Fastify + Node/TS backend
│   ├── web-storefront/    # React + Vite + Tailwind
│   ├── web-admin/         # React + Vite + Tailwind
│   ├── web-vendor/        # React + Vite + Tailwind
│   └── mobile/            # React Native (Expo bare)
├── packages/
│   ├── contracts/         # Shared Zod schemas + TS types (raw TS, no pre-compiled JS)
│   ├── api-client/        # Typed fetch wrapper (client.ts + client.native.ts)
│   ├── ui/                # Design tokens, Tailwind preset, primitive components
│   ├── config/            # tsconfig.base.json, eslint-preset.js, prettier config
│   └── seed/              # Demo preset data + importer script
├── turbo.json
├── pnpm-workspace.yaml
└── .env.example
```

All packages export **raw TypeScript** (no pre-compiled JS). Each consuming app transpiles via its own bundler (Vite/Metro). This eliminates ESM/CJS format mismatches.
...
**Note on Drizzle versioning:** drizzle-kit is in `1.0.0-beta` track. Use `drizzle-kit@beta`. The ORM (`drizzle-orm@0.45.x`) is stable and production-grade. This split is intentional by the Drizzle team.
...
**Search architecture constraint** (from ARCHITECTURE.md): Only schema-approved, searchable attributes from `attribute_definitions` may be projected into OpenSearch. No arbitrary runtime field creation from JSONB content.
...
**Critical architecture rule** (from ARCHITECTURE.md + PITFALLS.md): Neither SDK is imported directly in checkout or use-case code. Both are injected through the `PaymentProvider` interface:
...
Razorpay-specific constraints to implement (from PITFALLS.md): amounts in paise (integer), `captured` status on payment success, `on_hold` status for Route transfers.
...
**Fallback:** If Dinero.js v2 alpha stability is a concern, use plain BigInt arithmetic with a documented rounding/allocation utility in `packages/contracts/src/money/`. The PITFALLS.md requirement is BIGINT columns + integer minor units everywhere — the computation library is secondary.
...
Configure with `host: 'smtp.gmail.com', port: 587, auth: { type: 'OAuth2' }` or App Password. All email dispatch is event-driven via the `notifications` module.
...
Auth pattern: Fastify middleware validates JWT on every request; role claims drive module-level authorization gates; refresh token rotation stored in Redis with session invalidation support.
...
**Critical Metro config requirement** (from PITFALLS.md): Prove Metro workspace resolution in Phase 1. Configure `watchFolders` to include the monorepo root and all `packages/*`. Set `resolver.nodeModulesPaths`. Validate with a fresh `--reset-cache` build. Do not defer this.
...
**Recommended CI pipeline jobs per PR:**
1. `turbo lint` — ESLint across all packages (affected-only)
2. `turbo typecheck` — `tsc --noEmit` across all packages (affected-only)
3. `turbo test` — Vitest unit tests (affected-only)
4. `turbo build` — Production build check (affected-only)
...
# Backend (apps/api)
pnpm add fastify @fastify/cors @fastify/jwt zod drizzle-orm postgres ioredis bullmq awilix @fastify/awilix jose nodemailer stripe razorpay @opensearch-project/opensearch

pnpm add -D drizzle-kit @types/node vitest

# Web apps (apps/web-storefront, web-admin, web-vendor — run per app)
pnpm add react react-dom @tanstack/react-query zustand framer-motion

pnpm add -D vite @vitejs/plugin-react tailwindcss @tailwindcss/vite typescript @types/react @types/react-dom vitest

# React Native (apps/mobile)
pnpm add @react-navigation/native @react-navigation/stack @tanstack/react-query zustand react-native-reanimated
...
---
*Stack research for: Grovio — configurable multi-vendor marketplace starter kit*
*Researched: 2026-05-29*
